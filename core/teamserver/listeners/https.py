...
from core.utils import create_self_signed_cert, get_ipaddress, gen_random_string
...
class STListener(Listener):
    ...
    def run(self):
        ...
        if (
            ...
            and (self['Cert'] == 'data/cert.pem')
            and (
                not os.path.exists(self['Key'])
                or not os.path.exists(self['Cert'])
                or self['RegenCert']
            )
        ):
